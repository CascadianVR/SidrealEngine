...
void Renderer::Initialize()
{
    ...
    models[0].position = new float[3] {4.0f, 0.0f, 0.0f};

    models[1] = LoadModel("Resources\\CasOC\\CasOC.fbx");
    models[1].position = new float[3]{-3.0f, 0.0f, 0.0f};

    models[2] = LoadModel("Resources\\CUBE.obj");
    models[2].position = new float[3] {0.0f, 0.0f, 0.0f};
    
    models[3] = LoadModel("Resources\\Office.obj");
    models[3].position = new float[3] {-6.0f, 0.0f, 0.0f};
...
void Renderer::Render()
{
    ...
    for (int i = 0; i < 4; i++)
    {
        for (int j = 0; j < models[i].meshes.size(); j++)
        {
            ...
            Shader::SetShaderUniformVec3(&shaderProgram, "transform", models[i].position);
            //float* transform = MathUtils::Vec3toFloat3(Camera::GetCameraPosition());
            ...
            //if (&shaderProgram, models[i].meshes[j].textures.size() > 0)
            //{
